...
<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
		...
		<view class="search-form round">
			...
			<input v-model="searchForm.forumName" type="text" placeholder="论坛名称"></input>
		</view>
		<view class="action">
			<button @tap="search"
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
        ...
        <view style="float: right;">
            <text v-if="isAuth('forum','修改')" class="cuIcon-edit" @click.stop="onAddressDtailTap(item.id)"></text>
            <text v-if="isAuth('forum','删除')" style="margin-left: 50upx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	export default {
		...
		methods: {
			...
			async search() {
				...
				let res = await this.$api.list(`forum`, searchForm);
				// 如果是第一页数据置空
