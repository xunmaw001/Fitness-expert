   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					<view class="detail-list-item"
						...
						<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.jianshenkechengCollectionValue}}</view>
					</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('jianshenkechengCollection', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.jianshenkechengCollectionPhoto ? this.detail.jianshenkechengCollectionPhoto.split(",") : [];
			},
