<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
		...
		<view class="search-form round">
			...
			<input v-model="searchForm.jianshenkechengName" type="text" placeholder="健身课程名称"></input>
		</view>
		<view class="action">
			<button @tap="search"
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in jianshenkechengTypesList" :key="index"
	...
	<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
		<view @tap="onDetailTap(product)" class="uni-product"
			...
			<view class="uni-product-title"
				...
				{{product.jianshenkechengName}}</view>
			<view class="image-view">
				<image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
					class="uni-product-image" :src="product.jianshenkechengPhoto?product.jianshenkechengPhoto.split(',')[0]:''"></image>
			</view>
			<view class="uni-product-price" v-if="product.jianshenkechengNewMoney != null"
				:style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
				<text class="">￥{{product.jianshenkechengNewMoney}}</text>
			</view>
			<view style="display: flex;justify-content: space-between;" v-if="product.yonghuId == user.id">
				<text v-if="isAuth('jianshenkecheng','修改')" class="cuIcon-edit"
					@click.stop="onUpdateTap(product.id)">修改</text>
				<text v-if="isAuth('jianshenkecheng','删除')" class="cuIcon-delete"
	...
	<button
            ...
            v-if="isAuth('jianshenkecheng','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.jianshenkechengName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.jianshenkechengTypes = this.categoryName
                            }


				let res = await this.$api.list(`jianshenkecheng`, params);
				// 如果是第一页数据置空
			...
			async search() {
				...
				if (this.searchForm.jianshenkechengName) {
					searchForm['jianshenkechengName'] = this.searchForm.jianshenkechengName
				}
				let res = await this.$api.list(`jianshenkecheng`, searchForm);
				// 如果是第一页数据置空
